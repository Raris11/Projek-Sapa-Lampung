...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['proses_akun'])) {
    ...
    if ($action_type == 'tambah') {
        ...
        $password_hash = mysqli_real_escape_string($conn, password_hash($password, PASSWORD_DEFAULT));
        $query_user = "INSERT INTO users (nama, email, password, telepon, role, status) VALUES ('$nama', '$email', '$password_hash', '$telepon', 'petugas', '$status')";
        if (mysqli_query($conn, $query_user)) {
            ...
            $query_petugas = "INSERT INTO petugas (id_user, kode_petugas, divisi, kategori_petugas, wilayah, rating) VALUES ('$id_user_baru', '$kode_baru', '$divisi', '$divisi', 'Bandar Lampung', 0.0)";
        ...
        $password_sql = trim($password) !== '' ? ", password='" . mysqli_real_escape_string($conn, password_hash($password, PASSWORD_DEFAULT)) . "'" : "";
        $query_update = "UPDATE users SET nama='$nama', email='$email'$password_sql, telepon='$telepon', status='$status' WHERE id_user='$id_user'";
        if (mysqli_query($conn, $query_update)) {
            mysqli_query($conn, "UPDATE petugas SET divisi='$divisi', kategori_petugas='$divisi' WHERE id_user='$id_user'");
                        ...
                        <tbody>
                            <?php if($total_baris > 0) : ?>
                                ...
                                <tr class="<?= $rowClass ?>">
                                    ...
                                    <td><span class="pill pill-aktif">Password terenkripsi</span></td>
...
<div id="akunModal" class="modal">
    <div class="modal-content">
        ...
        <form action="akun.php" method="POST">
            ...
            <div class="ptg-fg" style="margin-bottom:12px;">
                <label id="password-label" style="display:block; font-size:12px; font-weight:bold; margin-bottom:5px;">Password Sistem *</label>
                <input type="password" name="password" id="in-password" class="ptg-input" style="width:100%; padding:10px; border-radius:6px; border:1px solid #cbd5e1;" required autocomplete="new-password">
            </div>
...
function openTambahModal() {
    ...
    document.getElementById('in-password').required = true;
    document.getElementById('password-label').innerText = "Password Sistem *";
...
function openEditModal(data) {
    ...
    document.getElementById('in-password').value = "";
    document.getElementById('in-password').required = false;
    document.getElementById('password-label').innerText = "Password Baru (opsional)";
