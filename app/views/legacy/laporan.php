...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_laporan'])) {
    ...
    $query_update = "UPDATE laporan SET status = '$status_baru', id_petugas = $val_petugas, petugas_id = $val_petugas WHERE id_laporan = '$id_lap_update'";
...
$cp = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as total FROM laporan WHERE status='Menunggu Verifikasi'"));
$cv = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as total FROM laporan WHERE status='Diverifikasi'"));
...
if (!empty($search)) {
    $sql .= " AND (laporan.id_laporan LIKE '%$search%' OR laporan.kode_tiket LIKE '%$search%' OR laporan.judul_laporan LIKE '%$search%')";
}
if (!empty($filter_kat)) {
...
$list_petugas = [];
$query_ptg = mysqli_query($conn, "SELECT petugas.id_petugas, petugas.divisi, COALESCE(NULLIF(petugas.kategori_petugas, ''), petugas.divisi) AS kategori_petugas, users.nama FROM petugas JOIN users ON petugas.id_user = users.id_user");
            ...
            <div class="card">
                ...
                <form method="GET" action="laporan.php">
                    <div class="filter-row">
                        <input type="text" name="search" class="form-input" placeholder="Cari tiket laporan atau judul..." value="<?= htmlspecialchars($search); ?>">
                        <select name="kategori" onchange="this.form.submit()">
                            ...
                            <option value="Sosial" <?= ($filter_kat == 'Sosial') ? 'selected' : ''; ?>>Sosial</option>
                            <option value="Pendidikan" <?= ($filter_kat == 'Pendidikan') ? 'selected' : ''; ?>>Pendidikan</option>
                            <option value="Kesehatan" <?= ($filter_kat == 'Kesehatan') ? 'selected' : ''; ?>>Kesehatan</option>
                        </select>
                    ...
                    <div style="margin-bottom: 20px;">
                        ...
                        <select name="id_petugas" id="form-petugas" class="form-input" style="width:100%; padding:8px; border-radius:6px;">
                            ...
                            <?php foreach($list_petugas as $p) : ?>
                                <option value="<?= $p['id_petugas']; ?>" data-kategori="<?= htmlspecialchars($p['kategori_petugas']); ?>">
                                    <?= htmlspecialchars($p['nama']); ?> (<?= htmlspecialchars($p['kategori_petugas']); ?>)
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
...
<script>
const petugasSelect = document.getElementById('form-petugas');
const allPetugasOptions = Array.from(petugasSelect.options).map(option => option.cloneNode(true));

function confirmLogout() {
    if (confirm("Yakin ingin keluar?")) { window.location.href = "../logout.php"; }
}

function filterPetugasByKategori(kategori, selectedPetugas) {
    petugasSelect.innerHTML = '';
    allPetugasOptions.forEach(option => {
        if (!option.value || option.dataset.kategori === kategori) {
            petugasSelect.appendChild(option.cloneNode(true));
        }
    });
    petugasSelect.value = selectedPetugas || '';
}

function openAdminModal(data) {
    document.getElementById('det-id').innerText = data.kode_tiket || data.id_laporan;
    ...
    filterPetugasByKategori(data.nama_kategori, data.id_petugas ? data.id_petugas : "");
